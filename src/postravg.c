...
/* postravg -- read data from a raw stream produced by hotm/demon and
   display  the running average */

/* started with postproc.c, v1.8, and uses the show.so module */

 static char * RCSId = "$Id: postravg.c 234 2016-10-06 22:58:14Z stanley $";
...
for( j=0; ; j++ ) {
	...
	if( !strncmp( argv[j], "-a", 2 ) ) {
		avgnum = atoi( argv[j+1] );
		printf( "using %d frame running average\n", avgnum );
		j++;
		continue;
	}
	...
	sum = calloc( pixCount, sizeof(struct _dpixel_) );

        if( SDL_Init( SDL_INIT_VIDEO ) < 0 ) {
                fprintf( stderr,
                                "%s: cannot init SDL: %s\n",
                                "postravg", SDL_GetError() );
                return -1;
        }
...
TOP:
	...
	/* incoming is in RGB. average and save */
	for( i=0; i< cameraModule.y*cameraModule.x; i++ ) {
		sum[i].r = (avgnum * sum[i].r + RGB[i].r) / (avgnum+1);
		sum[i].g = (avgnum * sum[i].g + RGB[i].g) / (avgnum+1);
		sum[i].b = (avgnum * sum[i].b + RGB[i].b) / (avgnum+1);
		RGB[i].r = sum[i].r;
		RGB[i].g = sum[i].g;
		RGB[i].b = sum[i].b;
	}
	...
	sprintf( fileName, "%ld.c%ld.snap.jpg", 
			baseEpoch,
