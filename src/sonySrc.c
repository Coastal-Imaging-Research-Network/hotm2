 ...
 *
 *  $Log: sonySrc.c,v $
 *  Revision 1.1  2004/12/06 21:52:56  stanley
 *  Initial revision
 *
...
static const char rcsid[] =
  "$Id: sonySrc.c 172 2016-03-25 21:03:51Z stanley $";
...
_init ()
{
  ...
  portNode = mapUIDToPortAndNode (cameraData.UID,
				  verbose & VERB_MAPPING, &cameraData.info);
  ...
  if (verbose & VERB_FORMAT) {
    ...
    printf ("cam %d data format %d\n",
	    cameraModule.cameraNumber, cameraModule.format);
  }
  ...
  if (i != cameraModule.left) {
    ...
    printf ("cam %d changed left from %d to %d\n",
	    cameraModule.cameraNumber, cameraModule.left, i);
    cameraModule.left = i;
  }
  ...
  if (i != cameraModule.x) {
    ...
    printf ("cam %d changed x from %d to %d\n",
	    cameraModule.cameraNumber, cameraModule.x, i);
    cameraModule.x = i;
  }
  ...
  if (i != cameraModule.top) {
    ...
    printf ("cam %d changed top from %d to %d\n",
	    cameraModule.cameraNumber, cameraModule.top, i);
    cameraModule.top = i;
  }
  ...
  if (i != cameraModule.y) {
    ...
    printf ("cam %d changed y from %d to %d\n",
	    cameraModule.cameraNumber, cameraModule.y, i);
    cameraModule.y = i;
  }
  ...
  for (i = 0; i < NUM_FEATURES; i++) {
	...
	&& (FEATURE_UNSET != cameraData.myFeatures[i])) {
      KIDID;
      printf ("cam %d %s: %s is ", cameraModule.cameraNumber, module, line);
  ...
  if (cameraData.format == FORMAT_SCALABLE_IMAGE_SIZE) {
    status = dc1394_dma_setup_format7_capture (cameraData.handle,
					       cameraData.myNode,
					       cameraModule.cameraNumber,
					       cameraData.mode,
					       cameraData.speed ==
					       400 ? SPEED_400 : (cameraData.
								  speed ==
								  200 ?
								  SPEED_200 :
								  SPEED_100),
    ...
    status = dc1394_dma_setup_capture (cameraData.handle, cameraData.myNode, cameraModule.cameraNumber, cameraData.format, cameraData.mode, cameraData.speed == 400 ? SPEED_400 : (cameraData.speed == 200 ? SPEED_200 : SPEED_100), cameraData.frameRate, 10,	/* number of buffers */
				       0,	/* dropframes? */
  ...
  if (verbose & VERB_CAMERA) {
    ...
    printf ("cam %d on port %d node %d (%d) device %s fd %d\n",
	    ...
	    cameraData.camera.dma_device_file, cameraData.camera.dma_fd);
    KIDID;
    ...
    printf ("cam %d last buffer: %d channel: %d \n",
	    cameraModule.cameraNumber,
	    cameraData.camera.dma_last_buffer, cameraData.camera.channel);
  }
  ...
  for (i = 0; i < NUM_FEATURES; i++) {
	...
	status = dc1394_start_one_push_operation (cameraData.handle,
						  cameraData.camera.node,
						  i + FEATURE_MIN);
	if (DC1394_SUCCESS != status) {
	...
	while (1) {
	  dc1394_is_one_push_in_operation (cameraData.handle,
					   cameraData.camera.node,
					   i + FEATURE_MIN, &yesNo);
	  if (yesNo == 0)
	    break;
 ...
 */

  {
	...
	printf ("cam %d starting autoexposure run, %d seconds\n",
		cameraModule.cameraNumber, autoSleep);
      }
	...
	status = dc1394_dma_single_capture (&cameraData.camera);
	_releaseFrame ();
      }
      clearAlarm ();
	...
	dc1394_get_feature_value (cameraData.handle,
				  cameraData.camera.node, FEATURE_GAIN, &i);
	dc1394_auto_on_off (cameraData.handle,
			    cameraData.camera.node, FEATURE_GAIN, 0);
	...
	dc1394_get_feature_value (cameraData.handle,
				  ...
				  FEATURE_SHUTTER, &i);
	dc1394_auto_on_off (cameraData.handle,
			    cameraData.camera.node, FEATURE_SHUTTER, 0);
	dc1394_set_feature_value (cameraData.handle,
				  cameraData.camera.node, FEATURE_SHUTTER, i);
      }
      if (FEATURE_AUTO == i3) {
	dc1394_get_white_balance (cameraData.handle,
				  cameraData.camera.node, &wb, &wr);
	dc1394_auto_on_off (cameraData.handle,
			    cameraData.camera.node, FEATURE_WHITE_BALANCE, 0);
    ...
    if (isTrigger) {
      ...
      status = dc1394_set_trigger_mode (cameraData.handle,
					cameraData.camera.node, trigMode);
      if (DC1394_SUCCESS != status) {
...
double
shutterToTime (char *name, long exp)
{
