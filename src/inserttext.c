...
static const char rcsid[] = "$Id: inserttext.c 172 2016-03-25 21:03:51Z stanley $";
...
void
insertText(int line, struct _pixel_ * data, char *textLine)
{
    int             i;
    int             j;
    int             k;
	...
	for (i = 0; i < strlen(textLine) * charWidth; i++) {
	    data[i + k * width + offset].r = 
	    data[i + k * width + offset].g =
	    data[i + k * width + offset].b = 0; 
	}
    }
	...
	for (j = 0; j < charHeight; j++) {
		...
		if (mask & b)
			data[(i * 8) + (j * width) + offset + k].r = 
			data[(i * 8) + (j * width) + offset + k].g =
			data[(i * 8) + (j * width) + offset + k].b = 255;
	    }
	}
    }
}
