...
static const char rcsid[] =
     "$Id: pleora.c 380 2018-09-05 12:58:29Z stanley $";
...
_init ()
{
     ...
     /* at this point, I know my camera number, I might need to do some external
        command. like start the Pleora camera */
     strcpy( line, getString(findCommand("cameraInitString")));
     if( strlen(line) ) {
	if(verbose) printf("cameraInitString: %s\n", line );
	printf("cameraInitString: %s\n", line );
	system(line);
     }
	...
	fakeCamera.model = &model[0];
     ...
     if (verbose & VERB_FORMAT) {
          KIDID;
          printf ("cam %ld data mode %ld interal format %d camera format %ld\n",
     ...
     /* handled by hotm.c exit handler */
